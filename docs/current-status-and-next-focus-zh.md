...
夜间 `night_ir` 在“当前摄像头、当前单点标定、固定红外黑白模式”的现场条件下已达到阶段性稳定基线。最新连续现场伪多点验证为有水花 `110/110`、无水花 `110/110`，合计 `220/220` 正确。该结论不外推到未标定的新点位、运行中彩色/IR 切换、多目标调度或不同天气条件。
...
- 控制面短超时：`turnPreset` 使用独立超时，仅对无响应网络错误重试一次
- FLV 受预算恢复：读流异常后最多重开 `2` 次，`500 ms -> 1000 ms` 退避，总预算 `10000 ms`
...
| 控制面调用 | connect `2000 ms`，read `3000 ms`，网络无响应重试 `1` 次 |
| FLV 故障恢复 | open `6000 ms`，read `3000 ms`，最多重开 `2` 次，总预算 `10000 ms` |

## 5. 下一步重点

当前单目标伪多点基线已完成 `220/220` 正确验证。下一步从“单点稳定观察”转为“多增氧机环境验证准备”：验证多个预置点、多个目标标定和配置下发后的调度边界。后续现场如再次出现失败，先根据 replay 的控制面调用、流恢复、sample-quality 拒绝诊断、硬门控计数和时序投票原因定位，不凭单轮现象直接改阈值。
...
- [2026-07-20 发布基线](./release-baseline-2026-07-20.md)
