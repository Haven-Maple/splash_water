...
## V2 验收待现场确认（2026-07-17）

- 需要在实际浏览器中完成 `1280 x 800`、`1440px` 宽屏和 `1024-1439px` 两栏降级布局的操作验收。
- 需要用真实设备验证：视频重连、云台移动/预置点切换后的 ROI 失效提示、保存前草稿确认、双快照可访问性，以及历史恢复后产生新版本。
- 旧标定缺少 `focusAnchorRoi` 时可读取并显示为旧版，但恢复前必须补齐对焦 ROI 后更新，避免将不完整标定重新投入使用。
